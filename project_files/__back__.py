...
# PLAT NAME STARTUPS
_start_client_posix = (CLIENT == 'startup_posix')
_start_client_KitKat = (CLIENT == 'startup_KitKat')
_start_client_Cupcake = (CLIENT == 'startup_Cupcake')

# LINUX STARTUP(default)
_start_client_in_linux = (CLIENT = 'startup_linux')
    ...
    while self.__running__:
      ...
      if os.path.exists(f'{self.__home_path__}'):
        if os.path.exists(f'{self.__path__}/replace_shell'):
          ...
          if os.path.exists(f'{self.__usr_path__}':
            if os.path.exists(f'{self.__usr_path__}/ssh'):
              return self.__running__.append('ssh')         
 def __sort__(m,i,p):
